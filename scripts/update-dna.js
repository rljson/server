...
const filesToBeDeleted = [
  ...
  'doc/workflows/update-from-server.md',
];

const templateRepo = 'https://github.com/rljson/server.git';
const localRepoPath = path.resolve(__dirname, '../../server');
...
function replaceServer() {
  ...
  for (const file of replaceFiles) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      content = content.replace(/server/g, projectName);
