...
/**
 * Normalizes Bs.getBlobStream() to always return a Web ReadableStream with getReader().
 * SocketMock/Socket.IO acks can deliver Buffer or node streams; this wrapper coerces them
 * into web streams so stream-based tests can iterate consistently.
 */
...
describe('[SocketMock] Data Storage (Io) Integration', () => {
  ...
  beforeEach(async () => {
    // Remove all event listeners from previous tests to prevent interference
...
describe('[SocketMock] Db Operations over IoMulti Integration', () => {
  let socketIoServer: SocketIoServer;
  let serverSockets: ServerSocket[] = [];
  let clientSockets: ClientSocket[] = [];
  const clientCount = 2;
...
describe('[SocketMock] Bs Operations over BsMulti Integration', () => {
  let socketIoServer: SocketIoServer;
  let serverSockets: ServerSocket[] = [];
  let clientSockets: ClientSocket[] = [];
  const clientCount = 2;
  ...
  beforeEach(async () => {
    // Remove listeners from previous tests to avoid accumulation
    serverSockets.forEach((socket) => socket.removeAllListeners());
    clientSockets.forEach((socket) => socket.removeAllListeners());
    ...
    // Rebuild getBlobStream into web streams for tests
  ...
  describe('Pattern: Blob Isolation', () => {
    it('Should verify isolated blobs do not exist on other clients', async () => {
      ...
      // Client B can still resolve via server proxy pulling from Client A
      expect(await clientB.bs!.blobExists(blobId)).toBe(true);
    });
  });
  ...
  describe('Edge Cases: Blob Size Extremes', () => {
    ...
    it('Should handle multi-megabyte blobs via PULL', async () => {
      // 512KB blob (keeps runtime reasonable in CI)
      const largeBlob = Buffer.alloc(512 * 1024, 'X');
...
describe('[SocketMock] Tree Operations over IoMulti Integration', () => {
  let socketIoServer: SocketIoServer;
  let serverSockets: ServerSocket[] = [];
  let clientSockets: ClientSocket[] = [];
  const clientCount = 2;
...
describe('[SocketMock] Coverage helpers', () => {
