...
It is **not** the bootstrap heartbeat. The connector does not listen to this
event, so nothing is applied because of it; it only lets a client notice that
it and the hub disagree and keep disagreeing. That is what
`@rljson/fs-agent`'s anti-entropy needs — and a periodic heartbeat, which
goes through every client's apply path, was measured net-harmful for exactly
that reason.

- Nothing is sent before the hub holds a state
- The timer calls `.unref()` and is cleared by `tearDown()`
...
| `${route}:state`       | Server → Client | State beacon (`stateBeaconMs`); never processed by the connector |
